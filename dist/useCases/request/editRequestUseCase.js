...
exports.editRequestUseCase = editRequestUseCase;
const databaseManager_1 = require("../../libs/databaseManager");
async function editRequestUseCase({ id, status, respondedById, ispId, employeeId, }) {
    ...
    if (status && status !== currentRequest.status) {
        ...
        if (status === "RESOLVED" || status === "CANCELED") {
            ...
            const responseTimeHours = (now.getTime() - currentRequest.createdAt.getTime()) / (1000 * 60 * 60);
